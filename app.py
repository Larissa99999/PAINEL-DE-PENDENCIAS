...
def criar_fig_segmentado(df_seg, group_col, titulo, eh_valor=False):
    ...
    # Reordena pelo total visível (sem Pendente normal)
    df_seg = df_seg.copy()
    df_seg['_sort'] = df_seg['Seg_Vencido'] + df_seg['Seg_Entrega'] + df_seg['Seg_Just']
    df_seg = df_seg.sort_values('_sort', ascending=True).drop(columns=['_sort'])
    fig = go.Figure()
    ...
    # Anotações com TOTAL por fora da barra (à direita) — só segmentos visíveis (sem Pendente normal)
    df_seg['_Total_Visivel'] = df_seg['Seg_Vencido'] + df_seg['Seg_Entrega'] + df_seg['Seg_Just']
    if eh_valor:
        textos_totais = [format_brl(v) for v in df_seg['_Total_Visivel']]
    else:
        textos_totais = [f"{int(v)}" for v in df_seg['_Total_Visivel']]

    # Adiciona texto do total no final de cada barra (posição x = Total visível, com padding)
    x_max = df_seg['_Total_Visivel'].max() if len(df_seg) > 0 else 0
    for i, (categoria, total, txt) in enumerate(zip(df_seg[group_col], df_seg['_Total_Visivel'], textos_totais)):
        fig.add_annotation(
            x=total, y=categoria,
            text=f"<b>{txt}</b>",
            showarrow=False,
...
st.markdown(f"""<div style='background:linear-gradient(135deg,#0f1a2e,#152038);border:2px solid #ff8c42;border-radius:12px;padding:18px 22px;margin:24px 0 4px 0'>
    <div style='display:flex;justify-content:space-between;align-items:center'>
        <div>
            ...
            <span style='color:#ffa570;font-size:0.8rem'>Notas fiscais recebidas que ainda não foram lançadas no sistema</span>
        </div>
