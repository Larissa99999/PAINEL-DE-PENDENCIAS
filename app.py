...
pc_cols = [c for c in ['Tipo','Solicitante','Comprador','Fornecedor','Nº PC','Valor','STATUS APROV','Dt Entrega PC','⚠️ Prazo','Chave Sefaz','Justificativa','Observacao','Prazo_Resolucao','Responsavel'] if c in df_pcs.columns]
...
VAZIOS_NOME = ['', '—', 'nan', 'None', '⚠️ Pendente Identificação']

if len(_df_vencidos_alerta) > 0:
    # Coleta todas as pessoas únicas (Comprador + Solicitante)
    _todas_pessoas = set()
    if 'Comprador' in _df_vencidos_alerta.columns:
        _todas_pessoas.update([p for p in _df_vencidos_alerta['Comprador'].dropna().unique() if str(p).strip() not in VAZIOS_NOME])
    if 'Solicitante' in _df_vencidos_alerta.columns:
        _todas_pessoas.update([p for p in _df_vencidos_alerta['Solicitante'].dropna().unique() if str(p).strip() not in VAZIOS_NOME])
    _todas_pessoas = sorted(_todas_pessoas)

    if _todas_pessoas:
        _cols_email = st.columns(min(4, len(_todas_pessoas)))

        for i, pessoa in enumerate(_todas_pessoas):
            # Pega todos os processos onde a pessoa é Comprador OU Solicitante
            _mask_comp = _df_vencidos_alerta['Comprador'].astype(str).str.strip() == pessoa if 'Comprador' in _df_vencidos_alerta.columns else pd.Series([False]*len(_df_vencidos_alerta))
            _mask_solic = _df_vencidos_alerta['Solicitante'].astype(str).str.strip() == pessoa if 'Solicitante' in _df_vencidos_alerta.columns else pd.Series([False]*len(_df_vencidos_alerta))
            _pend = _df_vencidos_alerta[_mask_comp | _mask_solic].drop_duplicates(subset=[c for c in ['Fornecedor','Nº PC','Nº Nota','Valor'] if c in _df_vencidos_alerta.columns])
            ...
            for _idx, (_, _r) in enumerate(_pend.iterrows(), 1):
                ...
                # Mostra o outro responsável
                _comp = str(_r.get('Comprador', '')).strip()
                _sol = str(_r.get('Solicitante', '')).strip()
                
                if _comp == pessoa and _sol not in VAZIOS_NOME and _sol != pessoa:
                    _outro = f"Solic: {_sol}"
                elif _sol == pessoa and _comp not in VAZIOS_NOME and _comp != pessoa:
                    _outro = f"Comprador: {_comp}"
                elif _comp == pessoa:
                    _outro = f"Solic: {_sol}" if _sol not in VAZIOS_NOME else ""
                else:
                    _outro = f"Comprador: {_comp}" if _comp not in VAZIOS_NOME else ""

                _linha_txt = f"{_idx}. {_forn}\n    {_nf_d} | {_pc_d} | {_valor_fmt}"
                if _outro:
                    _linha_txt += f" | {_outro}"
                _linhas.append(_linha_txt)
                if _idx >= 10:
                    break
            ...
            _nome = pessoa.replace('.', ' ').title().split()[0]
...
{link_painel}"""
            ...
            _email_dest = EMAIL_OVERRIDES.get(pessoa, f"{pessoa}@{dominio_email}")
            ...
            _bid = f"email_{pessoa.replace('.','_')}"
            col_idx = i % min(4, len(_todas_pessoas))
            with _cols_email[col_idx]:
                if st.button(f"📋 Copiar — {pessoa}", key=_bid):
                ...
                st.markdown(f"""<a href="{_url}" target="_blank" class="email-btn">
                    <div style="font-size:12px;font-weight:500">✉️ {pessoa}</div>
