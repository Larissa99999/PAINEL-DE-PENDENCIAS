    ...
    if uploaded:
        ...
        try:
            ...
            if _wb_meta.properties.modified:
                _dt_upload = _wb_meta.properties.modified.strftime("%d/%m/%Y às %H:%M")
            _wb_meta.close()
            uploaded.seek(0)
        except:
            pass
...
with _fc1:
    _compradores_inline = ["Todos"] + sorted([x for x in df['Comprador'].dropna().unique().tolist() if x not in ['—','','nan','None','⚠️ Pendente Identificação']]) if 'Comprador' in df.columns else ["Todos"]
    _sel_comp_inline = st.selectbox("Comprador", _compradores_inline, key="filtro_comp_inline")
with _fc2:
    _solicitantes_inline = ["Todos"] + sorted([x for x in df['Solicitante'].dropna().unique().tolist() if x not in ['—','','nan','None','⚠️ Pendente Identificação']]) if 'Solicitante' in df.columns else ["Todos"]
    _sel_sol_inline = st.selectbox("Solicitante", _solicitantes_inline, key="filtro_sol_inline")
with _fc3:
    _filiais_inline = ["Todas"] + sorted([x for x in df['Filial'].dropna().astype(str).unique().tolist() if x not in ['—','']]) if 'Filial' in df.columns else ["Todas"]
    _sel_fil_inline = st.selectbox("Filial", _filiais_inline, key="filtro_fil_inline")

# Aplica filtros inline (sobrescrevem os da sidebar)
if _sel_comp_inline != "Todos":
    df_filtered = df_filtered[df_filtered['Comprador'] == _sel_comp_inline]
if _sel_sol_inline != "Todos":
    df_filtered = df_filtered[df_filtered['Solicitante'] == _sel_sol_inline]
if _sel_fil_inline != "Todas" and 'Filial' in df_filtered.columns:
    df_filtered = df_filtered[df_filtered['Filial'].astype(str) == _sel_fil_inline]
...
# Coluna destaque: Sem PC?
def calc_sem_pc(row):
    pc = str(row.get('Nº PC', '')).strip().lstrip('0')
    if pc in VAZIOS_PC:
        return '⚠️ SEM PC'
    return ''
df_display['🔍 PC'] = df_display.apply(calc_sem_pc, axis=1)
...
n_sem_pc = int(df_nfs.apply(calc_sem_pc, axis=1).str.contains('SEM PC').sum()) if n_nfs > 0 else 0

# Colunas da tabela 1: destaque SEM PC + alerta prazo
nf_cols = [c for c in ['🔍 PC','Tipo','Solicitante','Comprador','Fornecedor','Nº Nota','Nº PC','Valor','Dt Emissão','Dt Entrega PC','Vencimento','⚠️ Prazo','Filial','Chave Sefaz','Justificativa','Observacao','Prazo_Resolucao','Responsavel'] if c in df_nfs.columns]
...
pc_cols = [c for c in ['Tipo','Solicitante','Comprador','Fornecedor','Nº PC','Valor','Dt Emissão','Dt Entrega PC','Vencimento','⚠️ Prazo','Filial','Controle','Status','Justificativa','Observacao','Prazo_Resolucao','Responsavel'] if c in df_pcs.columns]
    ...
    if len(_responsaveis) > 0:
        ...
        for i, resp in enumerate(_responsaveis):
            ...
            for _, _r in _pend_resp.iterrows():
                ...
                linha = f"• {_forn}\nPC: {_pc_display} | NF: {_nota_display} | Valor: {_valor} | Vencimento: {_venc}{_outro_resp}"
            ...
            if _nfs_pendentes:
                ...
                _body_parts.append(
                    f"🚨 NOTAS FISCAIS EMITIDAS – PENDENTES DE LANÇAMENTO\n"
                    f"({len(_nfs_pendentes)} processo{'s' if len(_nfs_pendentes) > 1 else ''} | {format_brl(_val_nfs)})\n\n"
                    + '\n\n'.join(_nfs_pendentes)
                )
            ...
            if _pcs_pendentes:
                ...
                if _pcs_vencidos_email:
                    _pc_section.append(
                        f"🔴 PEDIDOS DE COMPRA VENCIDOS\n"
                        f"({len(_pcs_vencidos_email)} processo{'s' if len(_pcs_vencidos_email) > 1 else ''})\n\n"
                        + '\n\n'.join(_pcs_vencidos_email)
                    )
                if _pcs_outros_email:
                    _pc_section.append(
                        f"📋 DEMAIS PROCESSOS PENDENTES\n"
                        f"({len(_pcs_outros_email)} processo{'s' if len(_pcs_outros_email) > 1 else ''})\n\n"
            ...
            if _total_linhas > MAX_ITENS_EMAIL:
                ...
                if _sobra > 0:
                    ...
                    if _all_pcs:
                        _body_parts_limited.append(
                            f"🔴 PEDIDOS PENDENTES\n(mostrando {len(_all_pcs)} de {len(_pcs_pendentes)})\n\n"
                            + '\n\n'.join(_all_pcs)
                        )
            ...
            _subject = "Ação Necessária – Pendências em Aberto sob sua Responsabilidade"
            _rodape_restantes = f"\n\n📌 + {_restantes} processo(s) não listado(s) neste email. Consulte o painel para a lista completa." if _restantes > 0 else ''
            
            _destaque_nf = ""
            if _nfs_pendentes:
                _destaque_nf = """
⚠️ Destaque importante: Há nota(s) fiscal(is) já emitida(s) por fornecedores que ainda não foram lançadas no sistema.

Caso a mercadoria ou serviço já tenham sido entregues/executados, solicitamos que o lançamento seja realizado o quanto antes. Se a entrega ainda não ocorreu, é indispensável atualizar a previsão no painel de pendências para garantir a visibilidade e o correto acompanhamento dos processos."""

            _body = f"""Prezado(a) {resp.replace('.', ' ').title()},

Atualmente, existem {_n} pendência(s) em aberto, totalizando {format_brl(_val)}, sendo que parte delas já se encontra com prazo vencido e impacta diretamente o controle operacional e financeiro.
{_destaque_nf}
...
Solicitamos, por gentileza, que as tratativas sejam realizadas com prioridade e que a justificativa/posição atual seja registrada no painel abaixo:
{link_painel}

Processos sem atualização, justificativa ou definição de prazo continuarão sendo sinalizados como críticos para acompanhamento gerencial.

Contamos com sua atenção e apoio para regularização dos apontamentos.

Atenciosamente,
Controle de Pendências"""

            # Monta email — URL curta (só destinatário + assunto) + botão copiar corpo
            EMAIL_OVERRIDES = {
                "igor.costa": "igor.costa@sdbioflor.com.br",
            }
            _email_dest = EMAIL_OVERRIDES.get(resp, f"{resp}@{dominio_email}")
            
            # URL curta: só to + subject (sem body)
            _subject_short = "Ação Necessária – Pendências em Aberto sob sua Responsabilidade"
            _url_outlook = (
            ...
            _btn_id = f"copy_{resp.replace('.','_')}"

            col_idx = i % min(4, len(_responsaveis))
            with cols_alerta[col_idx]:
                # Botão "Copiar corpo" com Streamlit
                if st.button(f"📋 Copiar email — {resp}", key=_btn_id):
                    st.code(_body, language=None)
                    st.info(f"☝️ Copie o texto acima e cole no corpo do email.")
                
                st.markdown(f"""<a href="{_url_outlook}" target="_blank" class="email-btn">
                    <div style="font-size:12px;font-weight:500">✉️ Abrir Outlook — {resp}</div>
