...
if _col_resp in _df_vencidos_alerta.columns and len(_df_vencidos_alerta) > 0:
    ...
    if len(_responsaveis) > 0:
        ...
        for i, resp in enumerate(_responsaveis):
            ...
            # Monta lista numerada com todas as pendências
            _linhas_email = []
            for _idx, (_, _r) in enumerate(_pend_resp.iterrows(), 1):
                ...
                _nota_display = f"NF {_nota}" if _nota not in ['', '—', 'nan', 'None'] else 'Sem NF'
                ...
                _solicitante_email = str(_r.get('Solicitante', '')).strip()
                _solicitante_email = _solicitante_email if _solicitante_email not in ['', '—', 'nan', 'None', '⚠️ Pendente Identificação'] else '—'
                
                _linhas_email.append(
                    f"{_idx}. {_forn}\n"
                    f"    {_nota_display} | {_pc_display} | {_valor} | Solic: {_solicitante_email}"
                )
                if _idx >= 10:
                    break

            _lista_txt = '\n\n'.join(_linhas_email)
            _restantes = max(0, _n - 10)
