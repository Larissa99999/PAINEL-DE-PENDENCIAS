        ...
        for i, resp in enumerate(_responsaveis):
            ...
            for _, _r in _pend_resp.iterrows():
                ...
                if _nota_display != '—' and 'PENDENTE' in _status:
                    _nf_rows.append({'Fornecedor': _forn, 'NF': _nota_display, 'PC': _pc_display, 'Valor': _valor, 'Prazo': _venc, 'Outro Resp.': _outro})
                else:
                    _venc_chk = parse_data(_r.get('Vencimento', pd.NaT))
                    if pd.notna(_venc_chk) and _venc_chk < _agora_alerta:
                        _dias = (pd.Timestamp.now() - _venc_chk).days
                        _pc_venc_rows.append({'Fornecedor': _forn, 'PC': _pc_display, 'Valor': _valor, 'Prazo': _venc, 'Atraso': f'{_dias}d', 'Outro Resp.': _outro})
                    else:
                        _pc_outros_rows.append({'Fornecedor': _forn, 'PC': _pc_display, 'Valor': _valor, 'Prazo': _venc, 'Outro Resp.': _outro})

            # Função para montar tabela alinhada em texto
            def _montar_tabela(rows, colunas):
                if not rows:
                    return ''
                # Calcula largura de cada coluna
                widths = {}
                for col in colunas:
                    widths[col] = max(len(col), max(len(str(r.get(col, ''))) for r in rows)) + 1
                # Header
                header = ' | '.join(col.ljust(widths[col]) for col in colunas)
                sep = '-+-'.join('-' * widths[col] for col in colunas)
                # Rows
                lines = [header, sep]
                for r in rows:
                    lines.append(' | '.join(str(r.get(col, '')).ljust(widths[col]) for col in colunas))
                return '\n'.join(lines)

            # Monta corpo com tabelas
            _body_parts = []
            MAX_ITENS = 8
            ...
            if _nf_rows:
                ...
                _nf_show = _nf_rows[:MAX_ITENS]
                _nf_cols = ['Fornecedor', 'NF', 'PC', 'Valor', 'Prazo']
                if any(r.get('Outro Resp.', '') for r in _nf_show):
                    _nf_cols.append('Outro Resp.')
                _tabela_nf = _montar_tabela(_nf_show, _nf_cols)
                _body_parts.append(
                    f"🚨 NOTAS FISCAIS EMITIDAS – PENDENTES DE LANÇAMENTO\n"
                    f"({len(_nf_rows)} processo{'s' if len(_nf_rows) > 1 else ''} | {format_brl(_val_nfs)})\n\n"
                    f"{_tabela_nf}"
                )

            if _pc_venc_rows:
                _pc_v_show = _pc_venc_rows[:MAX_ITENS]
                _pc_v_cols = ['Fornecedor', 'PC', 'Valor', 'Prazo', 'Atraso']
                if any(r.get('Outro Resp.', '') for r in _pc_v_show):
                    _pc_v_cols.append('Outro Resp.')
                _tabela_pcv = _montar_tabela(_pc_v_show, _pc_v_cols)
                _body_parts.append(
                    f"🔴 PEDIDOS DE COMPRA VENCIDOS\n"
                    f"({len(_pc_venc_rows)} processo{'s' if len(_pc_venc_rows) > 1 else ''})\n\n"
                    f"{_tabela_pcv}"
                )

            if _pc_outros_rows:
                _pc_o_show = _pc_outros_rows[:MAX_ITENS]
                _pc_o_cols = ['Fornecedor', 'PC', 'Valor', 'Prazo']
                if any(r.get('Outro Resp.', '') for r in _pc_o_show):
                    _pc_o_cols.append('Outro Resp.')
                _tabela_pco = _montar_tabela(_pc_o_show, _pc_o_cols)
                _body_parts.append(
                    f"📋 DEMAIS PROCESSOS PENDENTES\n"
                    f"({len(_pc_outros_rows)} processo{'s' if len(_pc_outros_rows) > 1 else ''})\n\n"
                    f"{_tabela_pco}"
                )

            _total_linhas = len(_nf_rows) + len(_pc_venc_rows) + len(_pc_outros_rows)
            _restantes = max(0, _total_linhas - MAX_ITENS * 3)
            ...
            _rodape_restantes = f"\n\n📌 + {_restantes} processo(s) não listado(s). Consulte o painel para a lista completa." if _restantes > 0 else ''
