...
# Pre-compute categorias para usar nos KPIs
_agora_pre = pd.Timestamp.now()
_em_10d_pre = _agora_pre + pd.Timedelta(days=10)

# Cat1: NFs pendentes de lançamento
...
# Cat2: Vencidos
_mask_venc_pre = pd.Series([False]*len(df_filtered), index=df_filtered.index)
if 'Vencimento' in df_filtered.columns:
    _v_pre = df_filtered['Vencimento'].apply(parse_data)
    _mask_venc_pre = (_v_pre < _agora_pre) & ~_mask_nf_pre

# Cat3: Entrega encerrada
_mask_entr_pre = pd.Series([False]*len(df_filtered), index=df_filtered.index)
if 'Dt Entrega PC' in df_filtered.columns:
    _e_pre = df_filtered['Dt Entrega PC'].apply(parse_data)
    _mask_entr_pre = (_e_pre < _agora_pre) & ~_mask_nf_pre & ~_mask_venc_pre

_mask_ativos = _mask_nf_pre | _mask_venc_pre | _mask_entr_pre
