...
if _col_resp in _df_vencidos_alerta.columns and len(_df_vencidos_alerta) > 0:
    ...
    if len(_responsaveis) > 0:
        ...
        for i, resp in enumerate(_responsaveis):
            ...
            # Monta tabela simples com todas as pendências
            _rows = []
            for _, _r in _pend_resp.iterrows():
                ...
                _pc_display = f"PC {_pc}" if _pc not in ['', '—', 'nan', 'None'] else '⚠️ PC NÃO LOCALIZADO'
                _nota_display = _nota if _nota not in ['', '—', 'nan', 'None'] else '—'
                
                # Define status
                _status_email = str(_r.get('Status', '')).strip()
                _sit = str(_r.get('Situação', '')).strip()
                _tem_j = _r.get('_tem_just', False)
                
                if 'PENDENTE' in _status_email.upper():
                    _status_txt = 'Sem lançamento'
                elif 'Vencido' in _sit and not _tem_j:
                    _status_txt = 'Sem tratativa'
                elif _tem_j:
                    _status_txt = 'Aguardando atualização'
                else:
                    _status_txt = 'Pendente'
                
                _rows.append({
                    'Fornecedor': _forn,
                    'NF': _nota_display,
                    'Pedido de Compra': _pc_display,
                    'Valor': _valor,
                    'Status': _status_txt
                })

            # Monta tabela alinhada
            _colunas = ['Fornecedor', 'NF', 'Pedido de Compra', 'Valor', 'Status']
            _widths = {}
            for col in _colunas:
                _widths[col] = max(len(col), max((len(str(r.get(col, ''))) for r in _rows), default=0)) + 1
            
            _header = ' | '.join(col.ljust(_widths[col]) for col in _colunas)
            _sep = '-+-'.join('-' * _widths[col] for col in _colunas)
            _table_lines = [_header, _sep]
            for r in _rows[:10]:
                _table_lines.append(' | '.join(str(r.get(col, '')).ljust(_widths[col]) for col in _colunas))
            _tabela = '\n'.join(_table_lines)
            
            _restantes = max(0, len(_rows) - 10)
            _rodape_rest = f"\n\n... e mais {_restantes} pendência(s). Consulte o painel para a lista completa." if _restantes > 0 else ''

            # Nome curto (primeiro nome)
            _primeiro_nome = resp.replace('.', ' ').title().split()[0]

            _subject = "Pendências Fiscais _ Atualizar painel"

            _body = f"""{_primeiro_nome},

⚠️ As notas abaixo já foram emitidas pelos fornecedores e seguem pendentes de lançamento/regularização:

{_tabela}{_rodape_rest}
...
{link_painel}"""
            ...
            _subject_short = "Pendências Fiscais _ Atualizar painel"
            _url_outlook = (
