...
with st.sidebar:
    ...
    if uploaded:
        ...
        # Persiste em arquivo para sobreviver a reloads
        try:
            import json as _json
            _meta = {"filename": uploaded.name, "datetime": datetime.now().strftime("%d/%m/%Y às %H:%M")}
            with open("upload_meta.json", "w") as _f:
                _json.dump(_meta, _f)
        except:
            pass
...
import json as _json_header

# Lê metadata: primeiro de session_state, fallback de arquivo persistido
...
if not data_ref or not nome_arq:
    try:
        with open("upload_meta.json", "r") as _f:
            _meta = _json_header.load(_f)
            nome_arq = nome_arq or _meta.get("filename", "")
            data_ref = data_ref or _meta.get("datetime", "")
    except:
        pass
...
<div class="header-bar">
    ...
    <p>Base: {data_base} &nbsp;|&nbsp; Upload: {data_ref} &nbsp;|&nbsp; {len(df)} itens totais &nbsp;|&nbsp; {len(df_filtered)} filtrados</p>
</div>
""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════
# 🚨 NOTAS CRÍTICAS — Sem PC e Sem Lançamento (PENDENTE SF1)
# ══════════════════════════════════════════════════════════════════════
VAZIOS_PC = ['', '—', 'nan', 'None', 'NaN', '0', '000000']

def _is_pendente_sf1(row):
    """Identifica se a nota está PENDENTE de lançamento no SF1."""
...
if n_criticas > 0:
    ...
    <div style="background:linear-gradient(135deg,#3a0a0a,#5a1010);border:2px solid #ff4d6a;border-radius:16px;padding:24px 28px;margin-bottom:20px">
        <div style="display:flex;align-items:center;gap:16px;margin-bottom:16px">
            ...
            <div>
                ...
                <div style="font-size:0.85rem;color:#ff8080;margin-top:4px">Notas com status PENDENTE no SF1 e sem PC vinculado — requerem ação imediata</div>
            </div>
        </div>
    ...
    st.dataframe(
        df_criticas_show.style.format(
            {col: (format_brl if col == 'Valor' else fmt_data)
             for col in df_criticas_show.columns
             if col in ['Valor', 'Dt Emissão', 'Vencimento']}
        ),
