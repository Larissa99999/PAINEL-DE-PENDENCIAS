...
def criar_fig_segmentado(df_seg, group_col, titulo, eh_valor=False):
    ...
    # Anotações à direita da barra
    x_max = df_seg['_Total_Visivel'].max() if len(df_seg) > 0 else 0
    for _, row in df_seg.iterrows():
        total = row['_Total_Visivel']
        pct = f"{total / grand_total * 100:.0f}%" if grand_total > 0 else "0%"

        if eh_valor:
            txt = f"<b>{format_abrev(total)}</b> <span style='color:#8892a4'>({pct})</span>"
        else:
            txt = f"<b>{int(total)}</b> <span style='color:#8892a4'>proc. ({pct})</span>"

        fig.add_annotation(
            x=total, y=row[group_col],
    ...
    # Formata eixo X com k/mi para valores
    if eh_valor:
        fig.update_xaxes(
            tickvals=[v for v in [0, 100_000, 200_000, 500_000, 1_000_000, 2_000_000, 5_000_000] if v <= x_max * 1.2],
            ticktext=[format_abrev(v).replace('R$ ', '') for v in [0, 100_000, 200_000, 500_000, 1_000_000, 2_000_000, 5_000_000] if v <= x_max * 1.2]
        )
    ...
    if len(_responsaveis) > 0:
        ...
        for i, resp in enumerate(_responsaveis):
            ...
            for _, _r in _pend_resp.iterrows():
                ...
                _pc = str(_r.get('Nº PC', '—'))
                _nota = str(_r.get('Nº Nota', '—'))
                _valor = format_brl(_r.get('Valor', 0))
                _venc = fmt_data(_r.get('Vencimento', ''))
                _tipo = str(_r.get('Tipo', ''))
                _linhas.append(f"• {_forn} | PC: {_pc} | NF: {_nota} | Valor: {_valor} | Venc: {_venc}")
